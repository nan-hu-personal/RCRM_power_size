                ...
                box(width = 9,
                    ...
                    fluidRow(
                      ...
                      column(width = 4,
                             ...
                             div(style="height:40px;",h5(HTML(paste0("SD of random intercept: \U03C3",tags$sub(0))))),
                             numericInput("sig0", 
                                          "", 
                                          value = 7))
                    ),
                    fluidRow(
                      column(width = 4,
                             numericInput("sig01", 
                                          h5(HTML(paste0("Covariance of random intercept & slope: \U03C3",tags$sub("01")))), 
                                          value = 3.5)
                      ),
                      column(width = 4,
                             numericInput("sig1", 
                                          h5(HTML(paste0("SD of random slope: \U03C3",tags$sub(1)))), 
                                          value = 1.5)
                      ),
                      column(width = 4,
                             numericInput("sig", 
                    ...
                    fluidRow(
                      ...
                      column(width = 4,
                             numericInput("droprate", 
                                          h5("Dropout rate per assessment"), 
                                          value = 0.01)
                      )
                    )
                ),
...
######RCRM#########
  ...
  ppr <- eventReactive({
    ...
    for (i in 1:(n_t-2))
    {
      ...
      ti_mean<-mean(ti)
      ti2_mean<-mean(ti^2)
      Ci<-r*(i+1)*(sig^2*ti2_mean+(i+1)*sig0^2*(ti2_mean-ti_mean^2))/(sig^4+(i+1)*sig^2*(sig1^2*ti2_mean+sig0^2+2*sig01*ti_mean)+(i+1)^2*(sig0^2*sig1^2-sig01^2)*(ti2_mean-ti_mean^2))
      C<-C+Ci
    }
    ...
    C<-C+(1-r*(n_t-1))*n_t*(sig^2*t2_mean+n_t*sig0^2*(t2_mean-t_mean^2))/(sig^4+n_t*sig^2*(sig1^2*t2_mean+sig0^2+2*sig01*t_mean)+n_t^2*(sig0^2*sig1^2-sig01^2)*(t2_mean-t_mean^2))
    }
  ...
  output$choice<-renderText({
    txt = ""
    if (input$choice==1){
      txt = "Given power, find sample size:"
    }else{
      txt = "Given sample size, find power:"
    }
    paste(txt,ppr()[1])
  })
  ...
  rcrm_newRow<-reactive({
    test_type=ifelse(input$level==1,"one-sided","two_sided")
    ...
    row=data.frame(test_type,alpha,power,size,ratio,input$effect,input$sig0,input$sig01,input$sig1,input$sig,visit_time,input$droprate)
  ...
  observeEvent(input$subBtn,{
    ...
    colnames(rst_table)<-c("Test Type","\U03B1","Power","Total Sample Size","Allocation Ratio",
                     "Effect Size","sig0","sig01","sig1","sig","Assessment Time","Dropout Rate")
  ...
  output$tsm_choice<-renderText({
    txt = ""
    if (input$tsm_choice==1){
      txt = "Given power, total sample size is: "
    }else{
